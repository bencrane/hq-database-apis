...
/**
 * DELETE /api/hq/org/members/[user_id]?org_id={orgId}
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const result = OrgIdQuerySchema.safeParse(queryParams);
    ...
    if (error) {
      if (error.code === "PGRST116") {
        return corsJson({ error: { code: "NOT_FOUND", message: "Member not found in organization" } }, 404);
      }
      throw error;
    }
    ...
    console.error("DELETE /api/hq/org/members/[user_id] error:", error);
